...
class VecCache:
    def __init__(self, ttl: int, openai_api_key: str = "", embedding_model_name: str = "text-embedding-ada-002", vector_size: int = 1536):
        self.ttl = ttl
    ...
    def _generate_vector(self, text: str) -> list[float]:
        text = text.replace("\n", " ")
        return openai.Embedding.create(input = [text], model=self.embedding_model_name)['data'][0]['embedding']
    ...
    def search(self, text: str, return_with_distance: bool=False) -> Union[str, Tuple[str, float]]:
        vector = self._generate_vector(text)
